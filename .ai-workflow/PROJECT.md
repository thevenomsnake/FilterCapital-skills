...
- Problem: 简历可能来自文件、文本或不同招聘网站，仍需逐份阅读和记录，判断口径不统一。
- First value: 调用本地技能读取用户提供的简历来源，在 Codex 中生成每位候选人的独立调查与筛选结论。
...
批量筛选先逐人完成绝对判断，再用运行时随机源打乱展示；每条记录保留一个主评价维度、评价和理由，不生成候选人排名。
...
- [T001 生成候选人评审看板](tickets/T001-rank-resumes.md): completed, approved; historical, superseded by randomized independent decisions
...
- **实习生 fast-start 采用正向证据累积：** 系统训练、工具基础和具体课程、比赛、研究、公开数据或个人项目可以证明基础与迁移速度；既往生产流程只增强置信度，缺失时不扣分。岗位、专业和院系名称不能代替实际职责，只防止错误加分。
- **工作后长期退出再入学由 trajectory 独占：** 小公司为即时补充实习产能时，毕业后进入工作、随后约 18 个月以上无可验证实质活动再入学，触发路径连续性 stop 并覆盖总分；只有空档期内有日期和可核验的实际工作或相当强度相关产出才能推翻，不在 fit 重复扣分。
- **独立绝对判断，不做排名：** 每位候选人必须独立提供至少一条高区分度、可归因的正向证据链，证明训练或实践已经转化为岗位能力。学校、专业、课程、奖项、术语和多个模板项目不能靠累计跨线；每个人输出一个由决定性证据选择的主评价维度、评价和理由。所有独立判断完成后才随机打乱展示，顺序不代表优先级，整批无人推进是合法结果。
