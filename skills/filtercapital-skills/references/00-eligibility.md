...
Apply this gate only when the recruiter has explicitly confirmed the relevant education threshold for this hiring context. When it is not confirmed, mark the gate `not applicable` rather than importing it as a universal rule. If activated, decline when either condition is confirmed:
...
Treat this as an operating-cost gate, not evidence about intelligence, role capability, character, or future performance. Do not reuse it as a negative signal in later modules.
