...
5. Otherwise, treat capability `4+`, trajectory `3+`, and fit `3+` as an advance signal; use the inquiry bonus to distinguish stronger candidates.
6. Treat capability `3` with no hard failure as review unless role-specific evidence clearly supports advancement.
7. Treat capability `0-2` or fit `0-1` as decline unless the source material is incomplete, in which case return insufficient information.
...
2. advance, review, decline, or insufficient information with confidence;
3. decisive reasons, ordered by their effect on the hiring decision;
4. fact map;
5. anomalies and competing stories with material counterevidence;
6. score vector and gate outcomes;
